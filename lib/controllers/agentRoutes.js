  ...
  try {
    ...
    if (body && typeof(body.snapshot) === 'object') {
      ...
      if (connections && connections.length) {
        if (state.verbose) {
          logger.verbose(`postSensorReport received ${JSON.stringify(body, null, '  ')}`);
        } else {
    ...
    if (state.agentReports.priorReport) {
      ...
      snapshot.int = snapshot.int.filter(o => {
        ...
        if (connectionPrior) {
          ...
          return connectionPrior.dis || connectionPrior.snp || connectionPrior.tnp;
        }
