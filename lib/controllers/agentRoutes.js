...
exports.postSensorReport = function(req, res) {
  ...
  try {
    ...
    if (body && typeof(body.snapshot) === 'object') {
      ...
      logger.debug(`postSensorReport ${JSON.stringify(body, null, '  ')}`);
          ...
          connections.forEach(ic => { // ic is incoming connection update
            ...
            if (typeof(ic.charCount) !== 'undefined') {
              if (typeof(ic.charCount) === 'number') {
                ...
                if (charCount < 0) {
                  errors.push(`cannot report negative charCount (${charCount}) for interfaceUid ${icId}`);
                }
                ...
                if (/^-\d+$/.test(ic.charCount)) {
                  ...
                  errors.push(`expected number or parsable number string, got ${typeof(ic.charCount)} for interfaceUid ${icId}`);
                }
              } else {
                errors.push(`expected number or parsable number string, got ${typeof(ic.charCount)} for interfaceUid ${icId}`);
              }
            ...
            if (errors.length === 0) {
              ...
              let connection = state.connectionsAccum.find(o => o.interfaceUid === icId);
              if (connection) {
                if (charCount) {
      ...
      if (errors.length) {
        ...
        res.send(response);
        logger.verbose(`postSensorReport ${JSON.stringify(response)}`)
      }
    } else {
...
exports.getReport = function() {
  ...
  if (state.debug) {
    logger.debug(`sensorReport: ${JSON.stringify(snapshot)}`);
  }
  return snapshot;
};
//
